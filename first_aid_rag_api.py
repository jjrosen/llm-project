...
dense_index = pc.Index("first-aid")  # Use YOUR Pinecone index name
...
def index():
    return {
        "message": "First-Aid assistance Chatbot API (with RAG)",
        "endpoints": {
...
def create(chat_message: ChatMessage):
    ...
    if conversation_id not in conversations:
        conversations[conversation_id] = [
            {"role": "developer", "content": """You are an AI assistant designed to help first aid technicians with information to help diagnos and treat people with various injuries and you are knowledgable in first aid practices """}
        ]
    ...
    conversations[conversation_id].append({
        ...
        "content": f"""Here are excerpts from the official First-aid documentation: {documentation}. Use whatever
         ...
         to answer the following query: {user_message}. 
